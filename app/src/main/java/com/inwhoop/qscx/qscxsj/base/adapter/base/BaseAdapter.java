...
import com.inwhoop.qscx.qscxsj.base.adapter.Util;
...
public abstract class BaseAdapter<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    protected boolean isNeedAnimation = false;   //是否需要开启加载动画
    protected boolean isNeedBgChange = true;     //是否需要开启单双行不同颜色
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (showHeaderView && mHeaderViews.get(viewType) != null) {
            return ViewHolder.create(mContext, mHeaderViews.get(viewType));
        }
        ...
        switch (viewType) {
            ...
            case TYPE_EMPTY_VIEW:
                View itemView = LayoutInflater.from(mContext).inflate(R.layout.abs_footer_empty, null, false);
                ...
                viewHolder = ViewHolder.create(mContext, itemView);
                break;
            case TYPE_NODATA_VIEW:
                viewHolder = ViewHolder.create(mContext, new View(mContext));
                break;
            case TYPE_RELOAD_VIEW:
                viewHolder = ViewHolder.create(mContext, mReloadView);
                break;
        }
        return viewHolder;
    }
    ...
    public void setLoadMoreData(List<T> datas) {
        ...
        mDatas.addAll(datas);
//        notifyItemRangeChanged(size, mDatas.size());
    ...
    public void setNewData(List<T> datas) {
        ...
        if (datas.size() == 0) {
            ...
            mReloadView = null;
            addFooterView(mLoadingView);       //添加加载中布局
        }
    }
    ...
    public void setEmptyView(@LayoutRes int layoutId) {
        mEmptyView = LayoutInflater.from(mContext).inflate(layoutId, null, false);
        ViewGroup.LayoutParams params = new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
        mEmptyView.setLayoutParams(params);
    }
